...
from core.constant import GenConstant
from core.env import DataBaseConfig
from core.exception import ServiceWarning
from schemas.gen.gen_vo import GenTableModel, GenTableColumnModel
...
class TemplateInitializer:
    ...
    def init_jinja2(cls):
        ...
        """
        try:
            template_dir = os.path.join(os.getcwd(), 'apps/utils', 'gen_templates')
            env = Environment(
...
class TemplateUtils:
    ...
    # 项目路径
    FRONTEND_PROJECT_PATH = 'templates'
    BACKEND_PROJECT_PATH = 'xeapp'
    ...
    def get_file_name(cls, template: List[str], gen_table: GenTableModel):
        ...
        if 'controller.py.jinja2' in template:
            ...
            return f'{python_path}/repositories/{business_name}_dao.py'
        elif 'do.py.jinja2' in template:
            return f'{python_path}/models/sys/{business_name}_do.py'
        elif 'service.py.jinja2' in template:
            return f'{python_path}/services/{business_name}_service.py'
        elif 'vo.py.jinja2' in template:
            return f'{python_path}/schemas/sys/{business_name}_vo.py'
