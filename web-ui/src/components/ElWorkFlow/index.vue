...
<script setup>
    import {ref, inject, provide, reactive, nextTick, getCurrentInstance} from "vue";
    ...
    provide('select', proxy.selectHandle)
    ...
    const state = reactive({
        nodeConfig: props.modelValue,
        selectVisible: false
    })

    const {nodeConfig, selectVisible} = toRefs(state)

    watch(() => props.modelValue, value => state.nodeConfig = value)
    watch(() => state.nodeConfig, value => {
    ...
    const selectHandle = async (type, data) => {
        ...
        await nextTick(() => {
            proxy.$refs.useselect.open(type, data)
        })
    }
</script>
