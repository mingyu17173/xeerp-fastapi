<template>
    <div class="app-container">
        <header-view
                ...
                @search="handleQuery">
            <template v-slot:ft>
                <el-button type="primary" @click="CreateAdd" v-hasPermi="['erp:partner:add']">添加新客户</el-button>
            </template>
            ...
            <el-col :span="24">
                <el-table
                    ...
                    <el-table-column label="客户来源" align="center" prop="partnerFrom" width="120">
                        <template #default="scope">
                            {{ getCustomerFrom(scope.row.partnerFrom) }}
                        </template>
                    </el-table-column>

                    <el-table-column label="公司性质" align="center" prop="partnerNature" width="160">
                        <template #default="scope">
                            {{ getCustomerNature(scope.row.partnerNature) }}
                        </template>
                    </el-table-column>
                    <el-table-column label="公司类型" align="center" prop="partnerType" width="160">
                        <template #default="scope">
                            {{ getCustomerType(scope.row.partnerType) }}
                        </template>
                    </el-table-column>
                    <el-table-column label="客户行业" align="center" prop="partnerIndustry" width="160">
                        <template #default="scope">
                            {{ getCustomerIndustry(scope.row.partnerIndustry) }}
                        </template>
                    </el-table-column>
...
<script setup name="customer">
    ...
    function getTypePartnerScope(res) {
        if (res > 0) {
            return parseInt(res)
        }
        return 0
