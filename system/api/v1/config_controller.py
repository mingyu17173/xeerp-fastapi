...
from schemas.config_schema import ConfigModel, ConfigPageQueryModel, DeleteConfigModel
...
configRoute = APIRouter(prefix='/system/config', dependencies=[Depends(LoginService.get_current_user)])
...
@configRoute.post(
    '', 
    dependencies=[Depends(CheckUserInterfaceAuth('system:config:add'))]
)
...
@configRoute.put(
    '',
    dependencies=[Depends(CheckUserInterfaceAuth('system:config:edit'))]
)
...
@configRoute.delete(
    '/refreshCache', 
    dependencies=[Depends(CheckUserInterfaceAuth('system:config:remove'))]
)
@Log(title='参数管理', business_type=BusinessType.UPDATE)
...
@configRoute.get('/configKey/{config_key}')
...
@configRoute.post(
    '/export', 
    dependencies=[Depends(CheckUserInterfaceAuth('system:config:export'))]
)
