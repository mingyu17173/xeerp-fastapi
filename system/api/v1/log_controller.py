...
logRoute = APIRouter(prefix='/monitor', dependencies=[Depends(LoginService.get_current_user)])
...
@logRoute.delete(
    '/operlog/clean', 
    dependencies=[Depends(CheckUserInterfaceAuth('monitor:operlog:remove'))]
)
...
@logRoute.post('/operlog/export', dependencies=[Depends(CheckUserInterfaceAuth('monitor:operlog:export'))])
...
@logRoute.delete('/logininfor/clean', dependencies=[Depends(CheckUserInterfaceAuth('monitor:logininfor:remove'))])
...
@logRoute.post('/logininfor/export', dependencies=[Depends(CheckUserInterfaceAuth('monitor:logininfor:export'))])
