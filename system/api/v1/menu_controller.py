...
from schemas.menu_schema import DeleteMenuModel, MenuModel, MenuQueryModel
...
menuRoute = APIRouter(prefix='/system/menu', dependencies=[Depends(LoginService.get_current_user)])

@menuRoute.get('/treeselect')
...
@menuRoute.get('/roleMenuTreeselect/{role_id}')
...
@menuRoute.post('', dependencies=[Depends(CheckUserInterfaceAuth('system:menu:add'))])
...
@menuRoute.put('', dependencies=[Depends(CheckUserInterfaceAuth('system:menu:edit'))])
...
@menuRoute.delete('/{menu_ids}', dependencies=[Depends(CheckUserInterfaceAuth('system:menu:remove'))])
