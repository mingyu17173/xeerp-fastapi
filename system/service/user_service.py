...
from schemas.post_schema import PostPageQueryModel
...
from common.utils.common_util import CamelCaseUtil,SqlalchemyUtil
...
class UserService:
    ...
    @classmethod
    async def add_user_services(cls, request: Request, query_db: AsyncSession, page_object: AddUserModel):
        """
        新增用户信息service
        ...
        if not await cls.check_user_name_unique_services(query_db, page_object):
            ...
            try:
                ...
                user_data_list = await UserDao.get_user_by_all_list(query_db)
                for user in user_data_list:
                    dept = await DeptDao.get_dept_by_id(query_db, user.dept_id)
                    user.dept_item = model_to_dict(dept) if dept else None

                await request.app.state.redis.set(
                    f'{RedisInitKeyConfig.SYS_USER.key}:all',
                    json.dumps(
                        SqlalchemyUtil.serialize_result(user_data_list), ensure_ascii=False, default=str
                    ),
                )
    ...
    @classmethod
    async def edit_user_services(cls, request: Request, query_db: AsyncSession, page_object: EditUserModel):
        """
        编辑用户信息service
    ...
    @classmethod
    async def init_cache_sys_user_services(cls, query_db: AsyncSession, redis):
        """
        ...
        keys = await redis.keys(f'{RedisInitKeyConfig.SYS_USER.key}:all')
        # 删除匹配的键
        if keys:
