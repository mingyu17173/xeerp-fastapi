...
from core.constant import CommonConstant, MenuConstant
from core.enums import RedisInitKeyConfig
from core.env import AppConfig, JwtConfig
from core.get_db import get_db
from core.exception import LoginException, AuthException, ServiceException
from dao.login_dao import login_by_account
...
class LoginService:
    ...
    async def authenticate_user(cls, request: Request, query_db: AsyncSession, login_user: UserLogin):
        ...
        if not PwdUtil.verify_password(login_user.password, user[0].password):
            ...
            if password_error_count > 5:
                ...
                raise LoginException(message='10分钟内密码已输错超过5次，账号已锁定，请10分钟后再试')
            logger.warning('密码错误')
    ...
    async def __check_login_captcha(cls, request: Request, login_user: UserLogin):
        ...
        if not captcha_value:
            ...
            raise LoginException(message='验证码已失效')
        if login_user.code != str(captcha_value):
            logger.warning('验证码错误')
            raise LoginException(message='验证码错误')
        return True
